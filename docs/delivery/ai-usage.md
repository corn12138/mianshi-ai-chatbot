...
- 采用“mock 工具宽输入覆盖”的补强方向：政策 query 别名、自然语言待办、常见时区别名、中文数字/百分比/次方/隐式乘法等计算输入，让 mock 演示不只依赖固定模板。
...
- 对计算工具没有采用 `eval` / `Function` 这类快捷实现，而是拆成输入归一化和手写 parser；同时把工具路由从固定关键词扩展为更接近真实用户表达的规则。
...
- 拒绝只修单个被测出来的 case；对工具层按边界类别补测试和文档，避免 mock 看起来只会回答预设问题。
