...
| mock 覆盖更多内部员工场景 | 增强项，超出原题必选范围 | `apps/api/src/tools/hr-policy.tool.ts`、`apps/api/src/tools/calculator.tool.ts`、`apps/api/src/tools/tool-router.ts`、`apps/api/src/llm/mock-llm.provider.ts` |
| 工具边界与宽泛输入处理 | 增强项，超出原题必选范围 | `apps/api/src/tools/math-expression.ts`、`apps/api/src/tools/tool-router.ts`、`apps/api/src/tools/todo.tool.ts`、`apps/api/src/tools/time.tool.ts`、`docs/development/technical-design.md` |
...
| mock 工具覆盖面和边界处理 | `README.md`、`docs/delivery/solution.md`、`docs/development/technical-design.md` |
