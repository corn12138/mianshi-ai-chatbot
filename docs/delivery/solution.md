...
- 工具层：注册员工服务知识查询、待办创建、时间查询、受控数学表达式计算。
...
为了避免 mock 看起来只会命中固定模板，工具层额外补了常见边界：

- 政策工具支持受控 `topic` 和自然语言 `query`，例如“VPN 登不上怎么办”“我想在家办公怎么申请”。
- 计算工具不使用 `eval`，通过 `math-expression.ts` 归一化输入并解析受控表达式，支持 `2**3+2`、`二加三`、`2(3+4)`、百分比、全角符号和中文乘除加减。
- 待办工具会从“提醒我后天提交报销”“下周五同步项目风险”中拆出标题和截止时间，并做长度校验。
- 时间工具支持常见城市/时区别名，例如“纽约现在几点”会归一到 `America/New_York`。
- `ToolRegistry` 会把模型或网关传来的异常参数归一成对象，单个工具失败会结构化返回，不打断聊天主流程。
